...
export const specialist = [
  ...
  },
  {
    ...
    image:
      "https://img.freepik.com/fotos-premium/confiado-mi-capacidad-medica-retrato-recortado-atractiva-joven-doctora-pie-brazos-cruzados-oficina_590464-2228.jpg?semt=ais_hybrid&w=740&q=80",
  },
  {
    id: 4,
...
const storedSpecialist =
  JSON.parse(localStorage.getItem("specialist")) || specialist;
...
export const schedule = (newItem) => {
  ...
  const specialistIndex = storedSpecialist.findIndex(
    (s) => s.id === parseInt(newItem.specialistId)
  );
...
export const getByValidCode = (validCode) => {
  ...
  const agendaFinded = storedAgenda.find(
    (item) => item.validCode === validCode
  );
...
export const getSpecialistById = ({ id }) => {
  return storedSpecialist.find((s) => s.id === Number(id));
};
